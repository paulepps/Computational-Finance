...
text(sig.p.x, mu.p.x, labels="EQUAL WEIGHT", pos=4)
points(sig.p.y, mu.p.y, pch=16, col="black", cex=2.5)
text(sig.p.y, mu.p.y, labels="LONG-SHORT", pos=4)
...
text(sig.gmin, mu.gmin, labels="GLOBAL MIN", pos=2.5)
for (i in 1:length(x.msft)) {
  z.vec = c(x.msft[i], x.nord[i], x.sbux[i])
  mu.p = crossprod(z.vec,mu.vec)
  sig.p = sqrt(t(z.vec)%*%sigma.mat%*%z.vec)
...
text(sd.vec, mu.vec, labels=asset.names, pos=4)
text(sig.gmin, mu.gmin, labels="GLOBAL MIN", pos=2)        
text(sig.px, mu.px, labels="E1", pos=2) 
text(sig.py, mu.py, labels="E2", pos=2) 
...
text(sd.vals, mu.vals, labels=c("E1","E2","E3"), pos=2)
text(sig.gmin, mu.gmin, labels="GLOBAL MIN", pos=2)        
text(sd.vec, mu.vec, labels=asset.names, pos=4)
text(sig.pz.05, mu.pz.05, labels="E4", pos=2)    
...
plot(sqrt(sig2.z), mu.z, type="b", ylim=c(0, 0.06), xlim=c(0, 0.16), 
     pch=16, col="green", ylab=expression(mu[p]), xlab=expression(sigma[p]))
...
text(sd.vec, mu.vec, labels=asset.names, pos=4)
text(sig.t, mu.t, labels="tangency", pos=4)
text(sig.t.02, mu.t.02, labels="E1", pos=4)
text(sig.t.07, mu.t.07, labels="E2", pos=4)
